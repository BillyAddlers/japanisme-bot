...
module.exports = class CommandCore {
    ...
    parse(client, message) {
        if (this.devOnly && !client.config.owners.includes(message.author.id)) {
            message.channel.send(` ❌ **| ${message.author.toString()}, I can't obey your command, Papa!**`);
            return false;
        }
        if (this.authorPermission.length) {
            const perms = [];
            for (const perm of this.authorPermission) {
                if (!message.member.hasPermission(perm.toUpperCase()) || !client.config.owners.includes(message.author.id)) perms.push(perm);
            }
            if (perms.length) {
        ...
        if (this.clientPermission.length) {
            ...
            if (perms.length) {
                message.channel.send(`❌ **| ${message.author.toString()}, I don't have enough permissions to do this, Papa! \n** ${perms.map(x => `▫ | ${x}`).join("\n")}`);
                return false;
            }
        }
        if (this.cooldown) {
            const now = Date.now();
            if (!client.cooldowns.has(this.name)) client.cooldowns.set(this.name, new TreeMap());
            ...
            if (userCooldown && !client.config.owners.includes(message.author.id) && estimatedTime > 0) {
                message.channel.send(`⏱ **| ${message.author.toString()}, You can use this command again in \`${estimatedTime / 1000}s\`**, Papa!`).catch(e => message.channel.send(e.stack, { code: "ini" }));
                return false;
            }
